...
def process_aws(temp_output, output, timestamp, c_scan, s_scan):
    try:
        ...
        # Convert julian to string char
        julian_day_char = netCDF4.stringtochar(np.array([str(timestamp.timetuple().tm_yday)], 'S4'))

        # Add variable julian_day
        nc.createDimension('julian_day', 4)
        nc.createVariable('julian_day', 'S1', ('julian_day',))
        nc.variables['julian_day'][:] = julian_day_char
